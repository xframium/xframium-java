package org.xframium.integrations.sauceLabs.rest;
...
public class SauceREST implements Serializable {
    ...
    static {
        if (System.getenv("SAUCE_REST_ENDPOINT") != null) {
            ...
            BASE_URL = System.getProperty("saucerest-java.base_url", "http://saucelabs.com/");
        }
    }
    ...
    protected String getUserAgent() {
        String userAgent = "SauceREST/1.0" ;
    ...
    public byte[] downloadVideo(String jobId) {
        ...
        return downloadFile(jobId, restEndpoint);
    }
     ...
     * @param location represents the base directory where the video should be downloaded to
     */
    public byte[] downloadLog(String jobId) {
     ...
     * @param restEndpoint the URL to perform a HTTP GET
     */
    private byte[] downloadFile(String jobId, URL restEndpoint) {
        try {
            ...
            return ("Error downloading Sauce Results" + e.getMessage() ).getBytes();
        }
    ...
    public void updateJobInfo(String jobId, Map<String, Object> updates) {
        ...
        try {
            ...
            addAuthenticationProperty(postBack);
            String jsonText = JSONObject.wrap( updates ).toString();
