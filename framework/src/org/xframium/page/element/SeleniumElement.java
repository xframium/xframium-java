...
public class SeleniumElement extends AbstractElement {
	...
	public Element cloneElement() {
		...
		// return this;
		// }
		SeleniumElement element = new SeleniumElement(getBy(), getRawKey(), getElementName(), getPageName(),
				getContextElement(), locatedElement, index);
	 ...
	 *            the index
	 */
	private SeleniumElement(BY by, String elementKey, String fieldName, String pageName, String contextElement,
			WebElement locatedElement, int index) {
		super(by, elementKey, fieldName, pageName, contextElement);
...
//				} else {
				...
				if (imageData != null && imageData.length > 0) {
					try {
						...
						return fullImage.getSubimage(imageLocation.getX(), imageLocation.getY(), imageSize.getWidth(),
								imageSize.getHeight());
					} catch (Exception e) {
	...
	private Element getElement(String elementName) {
		...
		ElementDescriptor elementDescriptor = new ElementDescriptor(
				PageManager.instance(getWebDriver().getxFID()).getSiteName(), getPageName(), elementName);

		if (log.isDebugEnabled())
			log.debug(Thread.currentThread().getName() + ": Attempting to locate element using ["
					+ elementDescriptor.toString() + "]");

		Element myElement = PageManager.instance(getWebDriver().getxFID()).getElementProvider()
				.getElement(elementDescriptor);
		myElement.setDriver(getWebDriver());
		myElement = myElement.cloneElement();
		PageManager.instance(getWebDriver().getxFID()).getElementProvider().setCachedElement(myElement,
				elementDescriptor);
	...
	private ReportableBy useBy() {
		...
		case COMPLEX:
			if (subElementList != null && subElementList.size() > 0) {
				SubElement[] subList = getSubElement(((DeviceWebDriver) getWebDriver()).getAut(),
						((DeviceWebDriver) getWebDriver()).getPopulatedDevice().getOs().toLowerCase(),
						(DeviceWebDriver) getWebDriver());
				if (subList.length == 1) {
					//
					...
					throw new ScriptConfigurationException("Could not locate sub-element for " + getName() + "( "
							+ ((DeviceWebDriver) getWebDriver()).getPopulatedDevice().getOs() + ","
							+ ((DeviceWebDriver) getWebDriver()).getCloud().getProvider() + ", "
							+ ((DeviceWebDriver) getWebDriver()).getAut().getName() + " ("
							+ ((DeviceWebDriver) getWebDriver()).getAut().getVersion() + ") )");
				}
			} else {
	...
	private ByWrapper _useBy(BY byType, String keyValue) {
		...
		case XPATH:
			...
			if (getWebDriver().getPopulatedDevice().getOs() != null
					&& getWebDriver().getPopulatedDevice().getOs().toUpperCase().equals("IOS")) {
				if ((getWebDriver().getPopulatedDevice().getOsVersion() != null
						&& getWebDriver().getPopulatedDevice().getOsVersion().startsWith("11"))
						|| (getWebDriver().getCapabilities().getCapability(DriverFactory.AUTOMATION_NAME) != null
								&& getWebDriver().getCapabilities().getCapability(DriverFactory.AUTOMATION_NAME)
										.equals("XCUITest"))) {
		...
		if (webElement != null && webElement.getSize().getHeight() > 0 && webElement.getSize().getWidth() > 0) {
			if (getWebDriver() instanceof HasInputDevices) {
				...
				if (((DeviceWebDriver) getWebDriver()).getNativeDriver() instanceof AppiumDriver) {
					...
					Dimension windowSize = aP.translateDimension(getWebDriver(),
					...
					try {
						...
						((DeviceWebDriver) getWebDriver()).getCloud().getCloudActionProvider().tap(
								(DeviceWebDriver) getWebDriver(), new PercentagePoint(percentX, percentY, true),
								length);
					}
	...
	public boolean _release() {
		...
		if (webElement != null && webElement.getSize().getHeight() > 0 && webElement.getSize().getWidth() > 0) {
			if (getWebDriver() instanceof HasInputDevices) {
				...
				if (((DeviceWebDriver) getWebDriver()).getNativeDriver() instanceof AppiumDriver) {
					new TouchAction((AppiumDriver) ((DeviceWebDriver) getWebDriver()).getNativeDriver()).release()
							.perform();
				} else if (((DeviceWebDriver) getWebDriver()).getNativeDriver() instanceof RemoteWebDriver) {
					if (((DeviceWebDriver) getWebDriver()).getNativeDriver() instanceof HasTouchScreen)
	...
	protected Element[] _getAll() {
		...
		if (useBy != null) {
			...
			if (log.isInfoEnabled()) {
				if (webElements == null)
					log.info(Thread.currentThread().getName() + ": Could not locate by [" + getBy() + "] using ["
							+ getKey() + "]");
				else
			...
			for (int i = 0; i < webElements.size(); i++) {
				foundElements[i] = new SeleniumElement(getBy(), getKey() + "[" + (i + 1) + "]", getElementName(),
						getPageName(), getContextElement(), webElements.get(i), i);
	...
	private WebElement getElement() {
		...
		try {
			...
			if (useBy != null) {
				...
				if (log.isDebugEnabled()) {
					if (webElement == null)
						log.debug(Thread.currentThread().getName() + ": Could not locate by [" + getBy() + "] using ["
								+ getKey() + "]");
					else
	...
	protected boolean _isVisible() {
		...
		if (returnValue)
			getActionProvider().getSupportedTimers((DeviceWebDriver) getWebDriver(),
					getExecutionContext().getTimerName(), getExecutionContext(), null);
		return returnValue;
	}
	...
	protected void _setValue(String currentValue, SetMethod setMethod, String xFID) {
		...
		try {
			...
			if (!SetMethodFactory.instance().createSetMethod(webElement.getTagName()).set(webElement, getWebDriver(),
					currentValue, setMethod.name().toUpperCase(), xFID))
